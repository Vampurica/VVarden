...
let checkuser = async (interaction, load) => {
  ...
  if (load) {
    ...
    let procCheckUser = async (interaction, args, userID) => {
    ...
    if ('options' in interaction.data) {
      ...
      if (args.user != null) {
        ...
        procCheckUser(interaction, args, args.user);
      } else {
        // UserID Problably
        if (!isNaN(args.userid)) {
          // UserID Evals as Number
          procCheckUser(interaction, args, args.userid);
        } else {
          // Not a valid number
          procCheckUser(interaction, args, interaction.member.id);
        }
      }
    } else {
      // No args, check self
      procCheckUser(interaction, {}, interaction.member.id);
    }
  }
};
