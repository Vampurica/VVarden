...
// Checkuseradmin
let checkuseradmin = async (interaction, load) => {
  ...
  if (load) {
    ...
    let procCheckUserAdmin = async (interaction, args, userID) => {
    ...
    if (admin.includes(interaction.member.id)) {
      if ('options' in interaction.data) {
        ...
        if (args.user != null) {
          ...
          procCheckUserAdmin(interaction, args, args.user);
        } else {
          // UserID Problably
          if (!isNaN(args.userid)) {
            // UserID Evals as Number
            procCheckUserAdmin(interaction, args, args.userid);
          } else {
            // Not a valid number
